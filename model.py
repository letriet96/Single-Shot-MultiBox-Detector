...
def create_loc_conf(num_classes=21, bbox_aspect_num=None):
...
def non_max_suppression(boxes, scores, overlap: float):
    """
    Args:
        boxes: (Tensor) (num_bbox, 4) -> (xmin, ymin, xmax, ymax)
        scores: (Tensor) (num_bbox, ) -> (8732, )
        overlap_threshold: (Float)
    return: index những boxes có scores cao nhất được giữ lại
    """
    count = 0
    keep = scores.new(scores.size(0)).zero_().long()

    # Sắp xếp các bounding box theo độ tin cậy tăng dần
    value, idx = scores.sort(0)
    idx = idx[-200:]  # id của top 200 boxes có độ tự tin cao nhất (tăng dần)
    ...
    while idx.numel() > 0:
        ...
        # overlap area
        iou = jaccard(boxes[idx], boxes[i].unsqueeze(0))  # shape = (len(idx), 1)

        # Loại bỏ những box overlap với box chính được chọn
        mask = iou[:, 0] < overlap
        idx = idx[mask]
...
def nms(boxes, scores, overlap=0.45, top_k=200):
    ...
    scores: (Tensor) [num_box]
    """
    count = 0
    keep = scores.new(scores.size(0)).zero_().long()

    # boxes coordinate - tọa độ boxes
    x1 = boxes[:, 0]
    y1 = boxes[:, 1]
    x2 = boxes[:, 2]
    y2 = boxes[:, 3]
    ...
    value, idx = scores.sort(0)
    idx = idx[-top_k:]  # id của top 200 boxes có độ tự tin cao nhất (tăng dần)
    ...
    def __call__(self, loc_data, conf_data, dbox_list):
        ...
        for i in range(num_batch):
            ...
            for cl in range(1, num_classe):
                ...
                ids, count = non_max_suppression(boxes, scores, self.nms_thresh)
...
class SSD(nn.Module):
    def __init__(self, phase, cfg, nms_thresh):
        ...
        if phase == "inference":
            self.detect = Detect(nsm_thresh=nms_thresh)
