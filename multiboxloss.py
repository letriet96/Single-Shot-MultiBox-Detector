...
class MultiBoxLoss(nn.Module):
    ...
    def forward(self, predictions, targets):
        """
        Mục tiêu của hàm này là tính loss dựa trên box dự đoán thu được từ model và box thực tế
        Args:
            predictions: (List) [loc_data, conf_data, dbox_list] từ forward của class SSD
                        loc_data: (tensor) shape: [batch_size, num_boxes, 4]
                        conf_data: (tensor) shape: [batch_size, num_boxes, 21]
                        dbox_list: (tensor) Prior boxes in center-offset form. Shape: [num_priors, 4]
            targets: (Tensor) ground truth (xmin, ymin, xmax, ymax, label) -> (num_object, 5)
        return
        """
        ...
        loc_t = torch.Tensor(batch_size, num_dbox, 4).to(self.device)

        # Xét từng ảnh
        for idx in range(batch_size):
            truths_box = targets[idx][:, :-1].to(self.device)  # (xmin, ymin, xmax, ymax) BBox
            labels = targets[idx][:, -1].to(self.device)  # label

            dbox = dbox_list.to(self.device)  # (cx, cy, w, h)
            variances = [0.1, 0.2]
            match(self.jaccard_threshold, truths_box, dbox, variances, labels, loc_t, conf_t_label, idx)
        ...
        loss_conf = loss_conf.view(batch_size, -1)  # torch.size([num_batch, 8732])
